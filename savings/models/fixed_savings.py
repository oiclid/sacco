...
class FixedSavings(models.Model):
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="fixed_savings_accounts",
        
    )
    balance = models.DecimalField(max_digits=20, decimal_places=2, default=0)
    interest_rate = models.DecimalField(max_digits=5, decimal_places=2, default=5.0)  # monthly %
    ...
    def __str__(self):
        return f"FixedSavings #{self.pk} ({self.user.username})"
